...
const uint64_t IntHop::lineRateValues[10] = {10000000000lu, 20000000000lu, 25000000000lu,40000000000lu, 50000000000lu,100000000000lu,200000000000lu,400000000000lu,0,0};
...
uint32_t IntHeader::GetStaticSize(){
	if (mode == 0){
		...
		return sizeof(sendTs) + sizeof(remoteDelay) + sizeof(hopCnt);
	}else {
		return 0;
	}
}
