...
void Node::SwitchNotifyDequeue(uint32_t ifIndex, uint32_t inDev, uint32_t qIndex, Ptr<Packet> p){
	NS_ASSERT_MSG(false, "Calling NotifyDequeue() on a non-switch node or this function is not implemented");
}

void Node::SwitchReceivePFC(uint32_t ifIndex, uint32_t qIndex, uint32_t time){
	NS_ASSERT_MSG(false, "Calling ReceivePFC() on a non-switch node or this function is not implemented");
}
} // namespace ns3
