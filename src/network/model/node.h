...
public:
  ...
  virtual void SwitchNotifyDequeue(uint32_t ifIndex, uint32_t inDev, uint32_t qIndex, Ptr<Packet> p);
  virtual void SwitchReceivePFC(uint32_t ifIndex, uint32_t qIndex, uint32_t time);
};
