...
QbbHelper::Install (Ptr<Node> a, Ptr<Node> b)
{
  ...
  queueA->SetSwitchId(a->m_id);
  queueA->SetDevId(devA->GetIfIndex());
  queueA->SetQueueId(queue_id++);
  devA->SetQueue (queueA);
  Ptr<BEgressQueue> queueB = CreateObject<BEgressQueue> ();
  queueB->SetSwitchId(b->m_id);
  queueB->SetDevId(devB->GetIfIndex());
  queueB->SetQueueId(queue_id++);
...
void QbbHelper::QpDequeueCallback(FILE *file, Ptr<QbbNetDevice> dev, Ptr<const Packet> p, Ptr<RdmaQueuePair> qp){
	TraceFormat tr;
	GetTraceFromPacket(tr, dev, p, qp->m_qpid, Dequ, true);
	tr.Serialize(file);
}
