...
namespace ns3 {
	...
	void qbbHeader::SetRemoteDelay(uint64_t ts){
		NS_ASSERT_MSG(IntHeader::mode == 2, "qbbHeader cannot SetRemoteDelay when IntHeader::mode != 2");
		ih.remoteDelay = ts;
	}
	...
	uint32_t qbbHeader::GetBaseSize() {//12 bytes
		qbbHeader tmp;
