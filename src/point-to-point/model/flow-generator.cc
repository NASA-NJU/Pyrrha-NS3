...
Ptr<RandomVariableStream> FlowGenerator::GetEqualSizeStream (uint32_t seed, uint32_t& mean, uint32_t& maxx) { // KB
	...
	Settings::cdfFromFile.clear();
	Settings::cdfFromFile.push_back(std::make_pair(mean,0.0));
	Settings::cdfFromFile.push_back(std::make_pair(mean,1.0));
