...
void RdmaDriver::Init(void){
	...
	m_rdma->Setup(MakeCallback(&RdmaDriver::ReceiveComplete, this), MakeCallback(&RdmaDriver::MsgComplete, this));
}
...
void RdmaDriver::RemoveMessage(uint32_t qpid, uint32_t msgSeq){
	m_rdma->StopMessage(qpid, msgSeq);
}

void RdmaDriver::ReceiveComplete(Ptr<RdmaRxOperation> q){
	m_traceReceiveComplete(q);
}

void RdmaDriver::MsgComplete(Ptr<RdmaSndOperation> q){
