...
RdmaClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::RdmaClient")
	...
	.AddAttribute ("QPId",
				   "QP index",
				   UintegerValue (0),
				   MakeUintegerAccessor (&RdmaClient::m_qpid),
				   MakeUintegerChecker<uint32_t> ())
	.AddAttribute ("MSGSeq",
				   "message sequence",
				   UintegerValue (0),
				   MakeUintegerAccessor (&RdmaClient::m_msgSeq),
				   MakeUintegerChecker<uint32_t> ())
	.AddAttribute("TestFlow",
					"It is a test flow or not",
					BooleanValue(false),
					MakeBooleanAccessor(&RdmaClient::m_testFlow),
					MakeBooleanChecker())
	.AddAttribute ("SRCId",
				   "source id",
				   UintegerValue (0),
				   MakeUintegerAccessor (&RdmaClient::m_src),
				   MakeUintegerChecker<uint32_t> ())
	.AddAttribute ("DSTId",
				   "dst id",
				   UintegerValue (0),
				   MakeUintegerAccessor (&RdmaClient::m_dst),
				   MakeUintegerChecker<uint32_t> ())
	.AddAttribute ("FLOWId",
				   "flow id",
				   UintegerValue (0),
				   MakeUintegerAccessor (&RdmaClient::m_flow_id),
				   MakeUintegerChecker<uint32_t> ())
  ;
  return tid;
}
...
void RdmaClient::SetDstNode(Ptr<Node> dst_node){
	m_dst_node = dst_node;
}
...
void RdmaClient::StartApplication (void)
{
  ...
  rdma->AddQueuePair(m_size, m_pg, m_sip, m_dip, m_sport, m_dport, m_win, m_baseRtt, m_qpid, m_msgSeq, m_src, m_dst, m_flow_id, m_testFlow);
}
...
void RdmaClient::StopApplication ()
{
  ...
  Ptr<Node> node = GetNode();
  Ptr<RdmaDriver> rdma = node->GetObject<RdmaDriver>();
  rdma->RemoveMessage(m_qpid, m_msgSeq);
  // receiver
  rdma = m_dst_node->GetObject<RdmaDriver>();
  rdma->RemoveMessage(m_qpid, m_msgSeq);
}
