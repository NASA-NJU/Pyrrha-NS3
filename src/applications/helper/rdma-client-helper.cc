...
RdmaClientHelper::RdmaClientHelper (uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint64_t size, uint32_t win, uint64_t baseRtt, uint32_t qpId, uint32_t msgSeq, uint32_t src, uint32_t dst, uint32_t flowId, bool testFlow)
{
	...
	SetAttribute ("QPId", UintegerValue (qpId));
	SetAttribute ("MSGSeq", UintegerValue (msgSeq));
	SetAttribute ("SRCId", UintegerValue (src));
	SetAttribute ("DSTId", UintegerValue (dst));
	SetAttribute ("FLOWId", UintegerValue (flowId));
	SetAttribute ("TestFlow", BooleanValue (testFlow));
...
void
RdmaClientHelper::SetDstNode(Ptr<Node> dst_node){
	m_dst_node = dst_node;
}
